...
def load_and_process_image_fixed_test_col(
    img_index, ds_transforms, augmentation_transforms, target_sizes=None
):
    img_id = test_data[img_index]
...
class LoadAndProcessPosCol(object):
    ...
    def __call__(self, img_index):
        return load_and_process_image_pos_col(
            img_index, self.ds_transforms, self.augmentation_params, self.target_sizes
        )
...
class LoadAndProcessFixedTestCol(object):
    ...
    def __call__(self, img_index):
        return load_and_process_image_fixed_test_col(
            img_index,
            self.ds_transforms,
            self.augmentation_transforms,
            self.target_sizes,
...
def realtime_augmented_data_gen_neg_col(
    ...
    ds_transforms=ds_transforms_default,
    target_sizes=None,
    processor_class=LoadAndProcessNegCol,
):
    """
    new version, using Pool.imap instead of Pool.map, to avoid the data structure conversion
    from lists to numpy arrays afterwards.
...
def realtime_augmented_data_gen_pos_col(
    ...
    ds_transforms=ds_transforms_default,
    target_sizes=None,
    processor_class=LoadAndProcessPosCol,
):
    """
    new version, using Pool.imap instead of Pool.map, to avoid the data structure conversion
    from lists to numpy arrays afterwards.
...
def realtime_fixed_augmented_data_test_col(
    ...
    chunk_size=4000,
    target_sizes=None,
    processor_class=LoadAndProcessFixedTestCol,
):
    """
    ...
    for n in range(num_chunks):
        ...
        pool = mp.Pool(NUM_PROCESSES)
        gen = pool.imap(
            process_func, indices_n, chunksize=loadsize
        )  # lower chunksize seems to help to keep memory usage in check
