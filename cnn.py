...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, Flatten, Reshape, Input, Activation, Conv2D, MaxPooling2D, UpSampling2D, GaussianNoise, MaxPooling1D, Lambda, Concatenate
...
def main(model='resnet', mode='predict', num_chunks=num_chunks, chunk_size=chunk_size, input_sizes=input_sizes, batch_size=batch_size, nbands=nbands, model_name=model_name):   
	mode = 'train'
	...
	if mode=='train':
		...
		try:
			...
			multi_model.save_weights(model_name+'_weights_only{}.h5'.format(dt_string))
			print('interrupted!')
			print('saved weights')
